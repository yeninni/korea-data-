  ...
  return (
    <section id="about" className="bg-civic-900 py-14 text-white">
      <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:px-8">
        <div>
          <p className="text-sm font-black uppercase tracking-[0.2em] text-civic-100">{t.publicValue}</p>
          ...
          <div className="mt-6 grid gap-4">
            {publicDataSources.map((source) => (
              <article key={source.name} className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
                <h4 className="font-black text-slate-950">{localizeValue(source.name, t)}</h4>
                <p className="mt-1 text-sm font-semibold text-civic-700">
                  {localizeValue(source.provider, t)}
                </p>
                <p className="mt-3 text-sm leading-6 text-slate-600">{localizeValue(source.role, t)}</p>
              </article>
            ))}
          </div>
        </div>
